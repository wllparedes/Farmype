...
@section('title', 'Productos - Adulto mayor')
            ...
            <div class="header-body">
                ...
                <div class="row">
                    <div class="col-xl-3 col-lg-6">
                        <h1 class="text-white text-center">Adulto mayor</h1>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card shadow" id="productData">
                    ...
                    <div class="card-body">
                        <div class="row container-products">
                            @if ($productOlderAdults->isEmpty())
                                <h3 class="col-12 text-center">No hay productos en esta categoria</h3>
                            @else
                                @foreach ($productOlderAdults as $productOlderAdult)
                                    <div class="col-sm-6 col-md-6 col-lg-3 card-responsive-mobil">
                                        <div class="card">
                                            <div class="card-header">
                                                <div class="container-image">
                                                    <img class="card-img-top" src="{{ verifyImage($productOlderAdult->file) }}"
                                                        alt="{{ $productOlderAdult->name }}">
                                                </div>
                                            </div>
                                            <div class="card-body">
                                                <h4 class="card-title">{{ $productOlderAdult->name }}</h4>
                                                <p class="card-text product-description">
                                                    {{ $productOlderAdult->detail ? $productOlderAdult->detail : 'Sin detalle.' }}
                                                </p>
                                                <a class="btn btn-primary btn-sm w-100"
                                                    href="{{ route('client.product.view', $productOlderAdult->id) }}">Ver
                                                    más</a>
                                            </div>
                                        </div>
                                    </div>
